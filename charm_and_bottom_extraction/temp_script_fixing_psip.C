...
double mass_lo[mass_regions] = {3.45};
double mass_hi[mass_regions] = {4.25};

const char* mass_lo_name[mass_regions] = {"345"}; 
const char* mass_hi_name[mass_regions] = {"425"};
...
void temp_script_fixing_psip(){

    fout = new TFile("template_fits_fixing_psip.root","RECREATE");
    ...
    for(int i = 0; i < mass_regions; i++){
        ...
        TH1D* h1d_psip = (TH1D*)fin->Get(hname_psip);
        ...
        h1d_prompt = (TH1D*)h1d_psip->Clone();
        ...
        TH1D* h1d_para_prompt = new TH1D(Form("h1d_para_prompt_%s", region_name[i]),"Parameters Distribution",500,0.0,1);
        ...
        double initial_value = h1d_ee_FG_psip->Integral(h1d_ee_FG_psip->FindBin(mass_lo[i]+0.01), h1d_ee_FG_psip->FindBin(mass_hi[i]-0.01),"width")/h1d_ee_FG_unlike->Integral(h1d_ee_FG_unlike->FindBin(mass_lo[i]+0.01), h1d_ee_FG_unlike->FindBin(mass_hi[i]-0.01),"width");

        double scale_factor_central = func->GetParameter(1)/initial_value;

        TH1D* h1d_ee_FG_psip_central = (TH1D*)h1d_ee_FG_psip->Clone();
        h1d_ee_FG_psip_central->SetName("h1d_ee_FG_psip_central");
        h1d_ee_FG_psip_central->Scale(scale_factor_central);

        double scale_factor_lower = (func->GetParameter(1)-func->GetParameter(2))/initial_value;

        TH1D* h1d_ee_FG_psip_lower = (TH1D*)h1d_ee_FG_psip->Clone();
        h1d_ee_FG_psip_lower->SetName("h1d_ee_FG_psip_lower");
        h1d_ee_FG_psip_lower->Scale(scale_factor_lower);

        double scale_factor_upper = (func->GetParameter(1)+func->GetParameter(2))/initial_value;

        TH1D* h1d_ee_FG_psip_upper = (TH1D*)h1d_ee_FG_psip->Clone();
        h1d_ee_FG_psip_upper->SetName("h1d_ee_FG_psip_upper");
        h1d_ee_FG_psip_upper->Scale(scale_factor_upper);

        h1d_ee_FG_psip->Write();
        h1d_ee_FG_psip_central->Write();
        h1d_ee_FG_psip_lower->Write();
        h1d_ee_FG_psip_upper->Write();
