...
double mass_lo[mass_regions] = {1.45};
double mass_hi[mass_regions] = {2.80};

const char* mass_lo_name[mass_regions] = {"145"}; 
const char* mass_hi_name[mass_regions] = {"280"};
...
void temp_script_fixing_jpsi(){

    fout = new TFile("template_fits_systematic_on_BG_15percent.root","RECREATE");
    ...
    TFile* fbbbar = new TFile("/Users/vassu/Desktop/new_files/new_approach/likesign_analysis_alternate_approach.root","READ");
    ...
    for(int i = 0; i < mass_regions; i++){
        ...
        TH1D* h1d_jpsi = (TH1D*)fin->Get(hname_jpsi);
        ...
        for(int ii = 0; ii < numTemplates; ii++){
            ...
            if(ii == 3){
                ...
                initial_fraction[ii] = h1d_ee_FG_jpsi->Integral(h1d_ee_FG_jpsi->FindBin(mass_lo[i]+0.01), h1d_ee_FG_jpsi->FindBin(mass_hi[i]-0.01),"width")/h1d_ee_FG_unlike->Integral(h1d_ee_FG_unlike->FindBin(mass_lo[i]+0.01), h1d_ee_FG_unlike->FindBin(mass_hi[i]-0.01),"width");
        ...
        h1d_prompt = (TH1D*)h1d_jpsi->Clone();
        ...
        double initial_value = h1d_ee_FG_jpsi->Integral(h1d_ee_FG_jpsi->FindBin(mass_lo[i]+0.01), h1d_ee_FG_jpsi->FindBin(mass_hi[i]-0.01),"width")/h1d_ee_FG_unlike->Integral(h1d_ee_FG_unlike->FindBin(mass_lo[i]+0.01), h1d_ee_FG_unlike->FindBin(mass_hi[i]-0.01),"width");

        double scale_factor_central = func->GetParameter(1)/initial_value;

        TH1D* h1d_ee_FG_jpsi_central = (TH1D*)h1d_ee_FG_jpsi->Clone();
        h1d_ee_FG_jpsi_central->SetName("h1d_ee_FG_jpsi_central");
        h1d_ee_FG_jpsi_central->Scale(scale_factor_central);

        double scale_factor_lower = (func->GetParameter(1)-func->GetParameter(2))/initial_value;

        TH1D* h1d_ee_FG_jpsi_lower = (TH1D*)h1d_ee_FG_jpsi->Clone();
        h1d_ee_FG_jpsi_lower->SetName("h1d_ee_FG_jpsi_lower");
        h1d_ee_FG_jpsi_lower->Scale(scale_factor_lower);

        double scale_factor_upper = (func->GetParameter(1)+func->GetParameter(2))/initial_value;

        TH1D* h1d_ee_FG_jpsi_upper = (TH1D*)h1d_ee_FG_jpsi->Clone();
        h1d_ee_FG_jpsi_upper->SetName("h1d_ee_FG_jpsi_upper");
        h1d_ee_FG_jpsi_upper->Scale(scale_factor_upper);

        h1d_ee_FG_jpsi->Write();
        h1d_ee_FG_jpsi_central->Write();
        h1d_ee_FG_jpsi_lower->Write();
        h1d_ee_FG_jpsi_upper->Write();
