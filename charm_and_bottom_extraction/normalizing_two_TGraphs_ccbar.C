...
void normalizing_two_TGraphs_ccbar(int stat){


    TFile* file = new TFile("/Users/vassu/Desktop/new_files/data_files/producing_templates_without_20percent_reduction.root","READ");
    TH1D* h1d_ccbar = (TH1D*)file->Get("h1d_ee_FG_ccbar");
    h1d_ccbar->SetName("h1d_ccbar");

    TH1D* h1d_ccbar_plot = (TH1D*)file->Get("h1d_ee_FG_ccbar");
    h1d_ccbar_plot->SetName("h1d_ccbar_plot");
    double x_graph, y_graph;

    for(int ibin = 0; ibin < mass_regions; ibin++){

        int bin_lo = h1d_ccbar->FindBin(mass_lo[ibin] + 0.01);
        int bin_hi = h1d_ccbar->FindBin(mass_hi[ibin] - 0.01);

        double bin_content = 0; double bin_error = 0;

        for(int i = bin_lo; i <= bin_hi; i++){

            bin_content += h1d_ccbar->GetBinContent(i)*h1d_ccbar->GetBinWidth(i);
            bin_error += pow(h1d_ccbar->GetBinError(i)*h1d_ccbar->GetBinWidth(i), 2);
    ...
    for(int i = 0; i < (int)graph->GetN(); i++){
        ...
        sum_pythia += y_graph*weight*graph->GetErrorX(i);
    ...
    TH1D* hist = new TH1D("hist", "Distribution", 500, -10, 10);

    TFile* f1 = new TFile("plotting_background_ccbar_central.root","READ");
    ...
    for(int itr = 0; itr < 1e6; itr++){
        ...
        for (int i = 0; i < n; ++i) {
            ...
            double weight = 1.0/pow(graph_stat->GetErrorY(i)*graph->GetErrorX(i), 2);
    ...
    TH1D* h1d_ccbar_central = (TH1D*)h1d_ccbar_plot->Clone();
    h1d_ccbar_central->SetName("h1d_ccbar_central");
    h1d_ccbar_central->Scale(func1->GetParameter(1));

    TH1D* h1d_ccbar_lower = (TH1D*)h1d_ccbar_plot->Clone();
    h1d_ccbar_lower->SetName("h1d_ccbar_lower");
    h1d_ccbar_lower->Scale(func1->GetParameter(1) - func1->GetParameter(2));

    TH1D* h1d_ccbar_upper = (TH1D*)h1d_ccbar_plot->Clone();
    h1d_ccbar_upper->SetName("h1d_ccbar_upper");
    h1d_ccbar_upper->Scale(func1->GetParameter(1) + func1->GetParameter(2));
    ...
    h2d_delta_phi_pair_mass_ccbar_central->Scale(func1->GetParameter(1));
    ...
    h2d_delta_phi_pair_mass_ccbar_lower->Scale(func1->GetParameter(1) - func1->GetParameter(2));
    ...
    h2d_delta_phi_pair_mass_ccbar_upper->Scale(func1->GetParameter(1) + func1->GetParameter(2));

    TH2D* h2d_ee_FG_ccbar = (TH2D*)f2->Get("h2d_ee_FG_unlike_ccbar");
    ...
    hist_ccbar_central->Scale(func1->GetParameter(1));

    TH2D* hist_ccbar_lower = (TH2D*)hist_ccbar->Clone();
    hist_ccbar_lower->SetName("hist_ccbar_lower");
    hist_ccbar_lower->Scale(func1->GetParameter(1) - func1->GetParameter(2));

    TH2D* hist_ccbar_upper = (TH2D*)hist_ccbar->Clone();
    hist_ccbar_upper->SetName("hist_ccbar_upper");
    hist_ccbar_upper->Scale(func1->GetParameter(1) + func1->GetParameter(2));

   

    TGraphAsymmErrors* band = new TGraphAsymmErrors((int)h1d_ccbar_central->GetNbinsX());
    band->SetName("band");

    for (int i = 1; i <= (int)h1d_ccbar_central->GetNbinsX(); ++i) {

        double x = h1d_ccbar_central->GetBinCenter(i);
        double y = h1d_ccbar_central->GetBinContent(i);

        double y_up   = h1d_ccbar_upper->GetBinContent(i);
        double y_down = h1d_ccbar_lower->GetBinContent(i);
    ...
    TFile* fout = new TFile("normalizing_two_TGraphs_ccbar_sys.root", "RECREATE");
    fout->cd();
    graph->Write();
