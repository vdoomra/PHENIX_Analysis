...
void azimuthal_correlations(){
    ...
    mg->GetYaxis()->SetRangeUser(-2e-9, 30e-9);

    TLatex *   tex1 = new TLatex(0.5,32e-9,"Run 2015 p+p, #sqrt{s} = 200 GeV, |#eta| < 0.35");
    ...
    TLatex *   tex4 = new TLatex(0.5,10e-9, "p_{T}^{pair} < 5 GeV/c");
    ...
    TLatex *   tex5 = new TLatex(0.4,5e-9, "p_{T}^{trig, e} > 1.5 GeV/c");
    ...
    TFile* fstat = new TFile("heavy_flavor_continuum_stat.root","READ");
    TH2D* h2d_delta_phi_pair_mass_total_central_stat = (TH2D*)fstat->Get("h2d_delta_phi_pair_mass_total_central");
    h2d_delta_phi_pair_mass_total_central_stat->GetXaxis()->SetRangeUser(mass_lo, mass_hi);
    TH1D* h1d_delta_phi_total_central_stat = (TH1D*)h2d_delta_phi_pair_mass_total_central_stat->ProjectionY();
    h1d_delta_phi_total_central_stat->SetName("h1d_delta_phi_total_central_stat");

    TH2D* h2d_delta_phi_pair_mass_total_lower_stat = (TH2D*)fstat->Get("h2d_delta_phi_pair_mass_total_lower");
    h2d_delta_phi_pair_mass_total_lower_stat->GetXaxis()->SetRangeUser(mass_lo, mass_hi);
    TH1D* h1d_delta_phi_total_lower_stat = (TH1D*)h2d_delta_phi_pair_mass_total_lower_stat->ProjectionY();
    h1d_delta_phi_total_lower_stat->SetName("h1d_delta_phi_total_lower_stat");

    TH2D* h2d_delta_phi_pair_mass_total_upper_stat = (TH2D*)fstat->Get("h2d_delta_phi_pair_mass_total_upper");
    h2d_delta_phi_pair_mass_total_upper_stat->GetXaxis()->SetRangeUser(mass_lo, mass_hi);
    TH1D* h1d_delta_phi_total_upper_stat = (TH1D*)h2d_delta_phi_pair_mass_total_upper_stat->ProjectionY();
    ...
    TFile* fsys = new TFile("heavy_flavor_continuum_sys.root","READ");
    TH2D* h2d_delta_phi_pair_mass_total_central_sys = (TH2D*)fsys->Get("h2d_delta_phi_pair_mass_total_central");
    h2d_delta_phi_pair_mass_total_central_sys->GetXaxis()->SetRangeUser(mass_lo, mass_hi);
    TH1D* h1d_delta_phi_total_central_sys = (TH1D*)h2d_delta_phi_pair_mass_total_central_sys->ProjectionY();
    h1d_delta_phi_total_central_sys->SetName("h1d_delta_phi_total_central_sys");

    TH2D* h2d_delta_phi_pair_mass_total_lower_sys = (TH2D*)fsys->Get("h2d_delta_phi_pair_mass_total_lower");
    h2d_delta_phi_pair_mass_total_lower_sys->GetXaxis()->SetRangeUser(mass_lo, mass_hi);
    TH1D* h1d_delta_phi_total_lower_sys = (TH1D*)h2d_delta_phi_pair_mass_total_lower_sys->ProjectionY();
    h1d_delta_phi_total_lower_sys->SetName("h1d_delta_phi_total_lower_sys");

    TH2D* h2d_delta_phi_pair_mass_total_upper_sys = (TH2D*)fsys->Get("h2d_delta_phi_pair_mass_total_upper");
    h2d_delta_phi_pair_mass_total_upper_sys->GetXaxis()->SetRangeUser(mass_lo, mass_hi);
    TH1D* h1d_delta_phi_total_upper_sys = (TH1D*)h2d_delta_phi_pair_mass_total_upper_sys->ProjectionY();
    h1d_delta_phi_total_upper_sys->SetName("h1d_delta_phi_total_upper_sys");


    TGraphAsymmErrors* band_sys = new TGraphAsymmErrors((int)h1d_delta_phi_total_central_sys->GetNbinsX());
    band_sys->SetName("band_sys");

    for (int i = 1; i <= (int)h1d_delta_phi_total_central_sys->GetNbinsX(); ++i) {

        double x = h1d_delta_phi_total_central_sys->GetBinCenter(i);
        double y = h1d_delta_phi_total_central_sys->GetBinContent(i);

        double y_up   = h1d_delta_phi_total_upper_sys->GetBinContent(i);
        double y_down = h1d_delta_phi_total_lower_sys->GetBinContent(i);
    ...
    TH2D* h2d_delta_phi_pair_mass_ccbar_central = (TH2D*)fstat->Get("h2d_delta_phi_pair_mass_ccbar_central");
    h2d_delta_phi_pair_mass_ccbar_central->GetXaxis()->SetRangeUser(mass_lo, mass_hi);
    TH1D* h1d_delta_phi_ccbar_central = (TH1D*)h2d_delta_phi_pair_mass_ccbar_central->ProjectionY();
    h1d_delta_phi_ccbar_central->SetName("h1d_delta_phi_ccbar_central");

    TH2D* h2d_delta_phi_pair_mass_bbbar_central = (TH2D*)fstat->Get("h2d_delta_phi_pair_mass_bbbar_central");
    h2d_delta_phi_pair_mass_bbbar_central->GetXaxis()->SetRangeUser(mass_lo, mass_hi);
    TH1D* h1d_delta_phi_bbbar_central = (TH1D*)h2d_delta_phi_pair_mass_bbbar_central->ProjectionY();
    h1d_delta_phi_bbbar_central->SetName("h1d_delta_phi_bbbar_central");
    ...
    h1d_delta_phi_ccbar_central->SetLineColor(kRed);
    h1d_delta_phi_ccbar_central->SetLineStyle(2);
    h1d_delta_phi_ccbar_central->Draw("same hist");

    h1d_delta_phi_bbbar_central->SetLineColor(kGreen-7);
    h1d_delta_phi_bbbar_central->SetLineStyle(2);
    h1d_delta_phi_bbbar_central->Draw("same hist");
    ...
    leg->AddEntry(band_sys, "c#bar{c} + b#bar{b} #rightarrow e^{+}e^{-} (PYTHIA8) Sys.");
    leg->AddEntry(h1d_delta_phi_ccbar_central, "c#bar{c} #rightarrow e^{+}e^{-} (PYTHIA8)");
    leg->AddEntry(h1d_delta_phi_bbbar_central, " b#bar{b} #rightarrow e^{+}e^{-} (PYTHIA8)");
    leg->Draw("same");
