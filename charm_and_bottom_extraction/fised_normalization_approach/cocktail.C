...
void cocktail(){
    ...
    TLatex *   tex4 = new TLatex(3.5,8e-8, "p_{T}^{pair} < 5 GeV/c");
    ...
    TLatex *   tex5 = new TLatex(3.4,4e-8, "p_{T}^{trig, e} > 1.5 GeV/c");
    ...
    TFile* f2 = new TFile("heavy_flavor_continuum_stat.root","READ");
    TH1D* h1d_total_central = (TH1D*)f2->Get("hist_total_dataBinning_central");
    TH1D* h1d_total_lower = (TH1D*)f2->Get("hist_total_dataBinning_lower");
    TH1D* h1d_total_upper = (TH1D*)f2->Get("hist_total_dataBinning_upper");
    ...
    TFile* f3 = new TFile("heavy_flavor_continuum_sys.root","READ");
    TH1D* h1d_total_central_sys = (TH1D*)f3->Get("hist_total_dataBinning_central");
    h1d_total_central_sys->SetName("h1d_total_central_sys");
    TH1D* h1d_total_lower_sys = (TH1D*)f3->Get("hist_total_dataBinning_lower");
    h1d_total_lower_sys->SetName("h1d_total_lower_sys");
    TH1D* h1d_total_upper_sys = (TH1D*)f3->Get("hist_total_dataBinning_upper");
    h1d_total_upper_sys->SetName("h1d_total_upper_sys");

    TH1D* h1d_total_sys = new TH1D("h1d_total_sys","Mass Distribution", nbins_mass, mass_range);
    h1d_total_sys->Sumw2();

    for(int ibin = 0; ibin < nbins_mass; ibin++){
        
        double bin_center = (mass_range[ibin] + mass_range[ibin+1])/2.;
        h1d_total_sys->SetBinContent(ibin+1, h1d_total_central_sys->GetBinContent(h1d_total_central_sys->FindBin(bin_center)));
        h1d_total_sys->SetBinError(ibin+1, h1d_total_central_sys->GetBinContent(h1d_total_central_sys->FindBin(bin_center)) - h1d_total_lower_sys->GetBinContent(h1d_total_lower_sys->FindBin(bin_center)));

    }

    TFile* f4 = new TFile("plotting_BG_with_uncertainties.root","READ");
    TH1D* h1d_mlp_dataBG = (TH1D*)f4->Get("h1d_mlp_dataBG");
    ...
    for(int ibin=1; ibin < nbins+1; ibin++){
        ...
        double sys_hf = h1d_total_sys->GetBinError(ibin);

        sys_err[counter] = sqrt( pow(sys_jpsi, 2) + pow(sys_psip, 2) + pow(sys_bg, 2) + pow(sys_phi, 2) + pow(sys_hf, 2) );
    ...
    TH1D* h1d_ccbar = (TH1D*)f2->Get("hist_ccbar_dataBinning_central");
    TH1D* h1d_bbbar = (TH1D*)f2->Get("hist_bbbar_dataBinning_central");
    TH1D* h1d_drell_yan = (TH1D*)f2->Get("hist_drell_yan_dataBinning_central");

    h1d_ccbar->SetLineColor(kGreen-4);
    h1d_ccbar->SetLineStyle(2);
    h1d_ccbar->Draw("same hist");

    h1d_bbbar->SetLineColor(kRed-4);
    h1d_bbbar->SetLineStyle(2);
    h1d_bbbar->Draw("same hist");
    ...
    leg->AddEntry(h1d_ccbar, "c#bar{c} #rightarrow e^{+}e^{-} (PYTHIA8)");
    leg->AddEntry(h1d_bbbar, "b#bar{b} #rightarrow e^{+}e^{-} (PYTHIA8)");
    ...
    for(int ibin=1; ibin < nbins+1; ibin++){
        ...
        double sys_err_cocktail = sqrt( pow(sys_jpsi, 2) + pow(sys_psip, 2) + pow(sys_bg, 2) + pow(sys_phi, 2) + pow(sys_hf, 2) );
