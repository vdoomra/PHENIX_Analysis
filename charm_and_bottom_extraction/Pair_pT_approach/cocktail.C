...
const int color[nfiles] = { kRed-4, kGreen-7, kViolet};
...
void cocktail(){
    ...
    mg->GetXaxis()->SetRangeUser(0.90, 4.5);
    ...
    TLatex *   tex1 = new TLatex(1.2,1e-5,"Run 2015 p+p, #sqrt{s} = 200 GeV, |#eta| < 0.35");
    ...
    TLatex *   tex4 = new TLatex(2.5,1e-5, "p_{T}^{pair} < 5 GeV/c");
    ...
    TLatex *   tex5 = new TLatex(2.5,5e-5, "p_{T}^{trig, e} > 1.5 GeV/c");
    ...
    TFile* f2 = new TFile("scaling_hists.root","READ");
    TH1D* h1d_ccbar_central_stat = (TH1D*)f2->Get("h1d_ee_FG_ccbar_central");
    TH1D* h1d_ccbar_lower_stat = (TH1D*)f2->Get("h1d_ee_FG_ccbar_lower");
    TH1D* h1d_ccbar_upper_stat = (TH1D*)f2->Get("h1d_ee_FG_ccbar_upper");

    TH1D* h1d_bbbar_central_stat = (TH1D*)f2->Get("h1d_ee_FG_bbbar_central");
    TH1D* h1d_bbbar_lower_stat = (TH1D*)f2->Get("h1d_ee_FG_bbbar_lower");
    TH1D* h1d_bbbar_upper_stat = (TH1D*)f2->Get("h1d_ee_FG_bbbar_upper");

    TH1D* h1d_total_central = (TH1D*)h1d_ccbar_central_stat->Clone();
    h1d_total_central->SetName("h1d_total_central");
    h1d_total_central->Add(h1d_bbbar_central_stat);

    TH1D* h1d_total_lower = (TH1D*)h1d_ccbar_lower_stat->Clone();
    h1d_total_lower->SetName("h1d_total_lower");
    h1d_total_lower->Add(h1d_bbbar_lower_stat);

    TH1D* h1d_total_upper = (TH1D*)h1d_ccbar_upper_stat->Clone();
    h1d_total_upper->SetName("h1d_total_upper");
    h1d_total_upper->Add(h1d_bbbar_upper_stat);

    TH1D* h1d_total_stat = new TH1D("h1d_total_stat","Mass Distribution", nbins_mass, mass_range);
    h1d_total_stat->Sumw2();

    for(int ibin = 0; ibin < nbins_mass; ibin++){
        
        double bin_center = (mass_range[ibin] + mass_range[ibin+1])/2.;
        h1d_total_stat->SetBinContent(ibin+1, h1d_total_central->GetBinContent(h1d_total_central->FindBin(bin_center)));
        h1d_total_stat->SetBinError(ibin+1, h1d_total_central->GetBinContent(h1d_total_central->FindBin(bin_center)) - h1d_total_lower->GetBinContent(h1d_total_lower->FindBin(bin_center)));

    }

    TGraphAsymmErrors* band_stat_ccbar = new TGraphAsymmErrors((int)h1d_ccbar_central_stat->GetNbinsX());
    band_stat_ccbar->SetName("band_stat_ccbar");

    for (int i = 1; i <= (int)h1d_ccbar_central_stat->GetNbinsX(); ++i) {

        double x = h1d_ccbar_central_stat->GetBinCenter(i);
        double y = h1d_ccbar_central_stat->GetBinContent(i);

        double y_up   = h1d_ccbar_upper_stat->GetBinContent(i);
        double y_down = h1d_ccbar_lower_stat->GetBinContent(i);
    ...
    TGraphAsymmErrors* band_stat_bbbar = new TGraphAsymmErrors((int)h1d_bbbar_central_stat->GetNbinsX());
    band_stat_bbbar->SetName("band_stat_bbbar");

    for (int i = 1; i <= (int)h1d_bbbar_central_stat->GetNbinsX(); ++i) {

        double x = h1d_bbbar_central_stat->GetBinCenter(i);
        double y = h1d_bbbar_central_stat->GetBinContent(i);

        double y_up   = h1d_bbbar_upper_stat->GetBinContent(i);
        double y_down = h1d_bbbar_lower_stat->GetBinContent(i);
    ...
    TFile* f3 = new TFile("plotting_BG_with_uncertainties.root","READ");
    TH1D* h1d_mlp_dataBG = (TH1D*)f3->Get("h1d_mlp_dataBG");
    ...
    TH1D* h1d_drell_yan_temp = (TH1D*)f5->Get("h1d_ee_FG_drellyan");

    TH1D* h1d_drell_yan = new TH1D("h1d_drell_yan" , "Mass Distribution", nbins_mass, mass_range);
    h1d_drell_yan->Sumw2();
    ...
    for(int ibin=1; ibin < nbins+1; ibin++){
        ...
        sys_err[counter] = sqrt( pow(sys_jpsi, 2) + pow(sys_psip, 2) + pow(sys_bg, 2) + pow(sys_phi, 2) );
    ...
    graph_stat_cocktail->SetMarkerColor(kViolet);
    graph_stat_cocktail->SetLineColor(kViolet);

    TGraphErrors* graph_sys_cocktail = new TGraphErrors(counter, x, y, x_lohi, sys_err);
    graph_sys_cocktail->SetFillColorAlpha(kViolet, 0.3);
    graph_sys_cocktail->SetLineColor(kViolet);

    graph_stat_cocktail->Draw("psame");
    graph_sys_cocktail->Draw("e2same");

    TFile* f4 = new TFile("scaling_hists.root","READ");
    
    TH1D* h1d_ccbar_central_sys = (TH1D*)f4->Get("h1d_ee_FG_ccbar_central");

    TH1D* h1d_ccbar_lower_sys = (TH1D*)h1d_ccbar_central_sys->Clone();
    h1d_ccbar_lower_sys->SetName("h1d_ccbar_lower_sys");
    h1d_ccbar_lower_sys->Scale(1.0-0.3337);

    TH1D* h1d_ccbar_upper_sys = (TH1D*)h1d_ccbar_central_sys->Clone();
    h1d_ccbar_upper_sys->SetName("h1d_ccbar_upper_sys");
    h1d_ccbar_upper_sys->Scale(1.0+0.1756);
    

    TH1D* h1d_bbbar_central_sys = (TH1D*)f4->Get("h1d_ee_FG_bbbar_central");

    TH1D* h1d_bbbar_lower_sys = (TH1D*)h1d_bbbar_central_sys->Clone();
    h1d_bbbar_lower_sys->SetName("h1d_bbbar_lower_sys");
    h1d_bbbar_lower_sys->Scale(1.0-0.1272);

    TH1D* h1d_bbbar_upper_sys = (TH1D*)h1d_bbbar_central_sys->Clone();
    h1d_bbbar_upper_sys->SetName("h1d_bbbar_upper_sys");
    h1d_bbbar_upper_sys->Scale(1.0+0.2406);
    

    TGraphAsymmErrors* band_sys_ccbar = new TGraphAsymmErrors((int)h1d_ccbar_central_sys->GetNbinsX());
    band_sys_ccbar->SetName("band_sys_ccbar");

    for (int i = 1; i <= (int)h1d_ccbar_central_sys->GetNbinsX(); ++i) {

        double x = h1d_ccbar_central_sys->GetBinCenter(i);
        double y = h1d_ccbar_central_sys->GetBinContent(i);

        double y_up   = h1d_ccbar_upper_sys->GetBinContent(i);
        double y_down = h1d_ccbar_lower_sys->GetBinContent(i);
    ...
    TGraphAsymmErrors* band_sys_bbbar = new TGraphAsymmErrors((int)h1d_bbbar_central_sys->GetNbinsX());
    ...
    h1d_drell_yan->SetLineColor(kBlack);
    h1d_drell_yan->SetLineStyle(2);
    ...
    for(int ibin=1; ibin < nbins+1; ibin++){
        ...
        double sys_err_cocktail = sqrt( pow(sys_jpsi, 2) + pow(sys_psip, 2) + pow(sys_bg, 2) + pow(sys_phi, 2) );
