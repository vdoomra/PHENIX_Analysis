...
void plotting_pair_pt(){
    ...
    std::ifstream inFile_ccbar("/Users/vassu/Desktop/new_files/data_files/mlp_pair_pt_ccbar.txt");
    ...
    std::ifstream inFile_bbbar("/Users/vassu/Desktop/new_files/data_files/mlp_pair_pt_bbbar.txt");
    ...
    h1d_pair_pt_ccbar->Scale(0.70603);
    h1d_pair_pt_bbbar->Scale(0.97923);

    TH1D* h1d_pair_pt_ccbar_lower_stat = (TH1D*)h1d_pair_pt_ccbar->Clone();
    h1d_pair_pt_ccbar_lower_stat->SetName("h1d_pair_pt_ccbar_lower_stat");
    h1d_pair_pt_ccbar_lower_stat->Scale(1.0-0.2853);

    TH1D* h1d_pair_pt_ccbar_upper_stat = (TH1D*)h1d_pair_pt_ccbar->Clone();
    h1d_pair_pt_ccbar_upper_stat->SetName("h1d_pair_pt_ccbar_upper_stat");
    h1d_pair_pt_ccbar_upper_stat->Scale(1.0+0.2853);

    TH1D* h1d_pair_pt_bbbar_lower_stat = (TH1D*)h1d_pair_pt_bbbar->Clone();
    h1d_pair_pt_bbbar_lower_stat->SetName("h1d_pair_pt_bbbar_lower_stat");
    h1d_pair_pt_bbbar_lower_stat->Scale(1.0-0.3422);

    TH1D* h1d_pair_pt_bbbar_upper_stat = (TH1D*)h1d_pair_pt_bbbar->Clone();
    h1d_pair_pt_bbbar_upper_stat->SetName("h1d_pair_pt_bbbar_upper_stat");
    h1d_pair_pt_bbbar_upper_stat->Scale(1.0+0.3422);
    ...
    TH1D* h1d_pair_pt_total_lower_stat = (TH1D*)h1d_pair_pt_ccbar_lower_stat->Clone();
    h1d_pair_pt_total_lower_stat->SetName("h1d_pair_pt_total_lower_stat");
    h1d_pair_pt_total_lower_stat->Add(h1d_pair_pt_bbbar_lower_stat);

    TH1D* h1d_pair_pt_total_upper_stat = (TH1D*)h1d_pair_pt_ccbar_upper_stat->Clone();
    h1d_pair_pt_total_upper_stat->SetName("h1d_pair_pt_total_upper_stat");
    h1d_pair_pt_total_upper_stat->Add(h1d_pair_pt_bbbar_upper_stat);
    ...
    TH1D* h1d_pair_pt_ccbar_lower_sys = (TH1D*)h1d_pair_pt_ccbar->Clone();
    h1d_pair_pt_ccbar_lower_sys->SetName("h1d_pair_pt_ccbar_lower_sys");
    h1d_pair_pt_ccbar_lower_sys->Scale(1.0-0.3337);

    TH1D* h1d_pair_pt_ccbar_upper_sys = (TH1D*)h1d_pair_pt_ccbar->Clone();
    h1d_pair_pt_ccbar_upper_sys->SetName("h1d_pair_pt_ccbar_upper_sys");
    h1d_pair_pt_ccbar_upper_sys->Scale(1.0+0.1765);

    TH1D* h1d_pair_pt_bbbar_lower_sys = (TH1D*)h1d_pair_pt_bbbar->Clone();
    h1d_pair_pt_bbbar_lower_sys->SetName("h1d_pair_pt_bbbar_lower_sys");
    h1d_pair_pt_bbbar_lower_sys->Scale(1.0-0.1272);

    TH1D* h1d_pair_pt_bbbar_upper_sys = (TH1D*)h1d_pair_pt_bbbar->Clone();
    h1d_pair_pt_bbbar_upper_sys->SetName("h1d_pair_pt_bbbar_upper_sys");
    h1d_pair_pt_bbbar_upper_sys->Scale(1.0+0.2406);
    ...
    TH1D* h1d_pair_pt_total_lower_sys = (TH1D*)h1d_pair_pt_ccbar_lower_sys->Clone();
    h1d_pair_pt_total_lower_sys->SetName("h1d_pair_pt_total_lower_sys");
    h1d_pair_pt_total_lower_sys->Add(h1d_pair_pt_bbbar_lower_sys);

    TH1D* h1d_pair_pt_total_upper_sys = (TH1D*)h1d_pair_pt_ccbar_upper_sys->Clone();
    h1d_pair_pt_total_upper_sys->SetName("h1d_pair_pt_total_upper_sys");
    h1d_pair_pt_total_upper_sys->Add(h1d_pair_pt_bbbar_upper_sys);

    TGraphAsymmErrors* band_ccbar_stat = new TGraphAsymmErrors((int)h1d_pair_pt_ccbar->GetNbinsX());
    band_ccbar_stat->SetName("band_ccbar_stat");

    for (int i = 1; i <= (int)h1d_pair_pt_ccbar->GetNbinsX(); ++i) {

        double x = h1d_pair_pt_ccbar->GetBinCenter(i);
        double y = h1d_pair_pt_ccbar->GetBinContent(i);

        double y_up   = h1d_pair_pt_ccbar_upper_stat->GetBinContent(i);
        double y_down = h1d_pair_pt_ccbar_lower_stat->GetBinContent(i);
    ...
    TGraphAsymmErrors* band_ccbar_sys = new TGraphAsymmErrors((int)h1d_pair_pt_ccbar->GetNbinsX());
    ...
    for (int i = 1; i <= (int)h1d_pair_pt_ccbar->GetNbinsX(); ++i) {
        ...
        double y_up   = h1d_pair_pt_ccbar_upper_sys->GetBinContent(i);
        double y_down = h1d_pair_pt_ccbar_lower_sys->GetBinContent(i);
    ...
    TGraphAsymmErrors* band_bbbar_stat = new TGraphAsymmErrors((int)h1d_pair_pt_bbbar->GetNbinsX());
    band_bbbar_stat->SetName("band_bbbar_stat");

    for (int i = 1; i <= (int)h1d_pair_pt_bbbar->GetNbinsX(); ++i) {

        double x = h1d_pair_pt_bbbar->GetBinCenter(i);
        double y = h1d_pair_pt_bbbar->GetBinContent(i);

        double y_up   = h1d_pair_pt_bbbar_upper_stat->GetBinContent(i);
        double y_down = h1d_pair_pt_bbbar_lower_stat->GetBinContent(i);
    ...
    TGraphAsymmErrors* band_bbbar_sys = new TGraphAsymmErrors((int)h1d_pair_pt_bbbar->GetNbinsX());
    ...
    for (int i = 1; i <= (int)h1d_pair_pt_bbbar->GetNbinsX(); ++i) {
        ...
        double y_up   = h1d_pair_pt_bbbar_upper_sys->GetBinContent(i);
        double y_down = h1d_pair_pt_bbbar_lower_sys->GetBinContent(i);
    ...
    leg->AddEntry(band_stat, "c#bar{c} + b#bar{b} #rightarrow e^{+}e^{-} (PYTHIA8)");
