...
void template_fits_pair_pt_sys(){
    ...
    TH1D* h1d_pair_pt_data_sys_original = (TH1D*)f2->Get("h1d_signal_sys");
    h1d_pair_pt_data_sys_original->Scale(1./h1d_pair_pt_data_sys_original->Integral("width"));
   ...
   h1d_pair_pt_bbbar_temp->Scale(10.);
    ...
    TH1D* h1d_iterative = new TH1D("h1d_iterative", "Distribution", nbins_mass, mass_range);
    h1d_iterative->Sumw2();

    for(int ibin = 1; ibin < nbins_mass+1; ibin++){

        h1d_iterative->SetBinContent(ibin, h1d_pair_pt_data_sys_original->GetBinContent(ibin) + h1d_pair_pt_data_sys_original->GetBinError(ibin));
        h1d_iterative->SetBinError(ibin, h1d_pair_pt_data_original->GetBinError(ibin));

    }

    h1d_iterative->Scale(1./h1d_iterative->Integral("width"));
        
    for(int itr = 0; itr < 1e5; itr++){

        hData.clear();

        int nbins = h1d_iterative->GetNbinsX();

        for (int ii = 1; ii <= nbins; ++ii) { hData.push_back(gRandom->Gaus(h1d_iterative->GetBinContent(ii), h1d_iterative->GetBinError(ii))); }
    ...
    for(int j = 0; j < parameters_bbbar.size(); j++) h1d_para_bbbar->Fill(parameters_bbbar[j]);

    TH1D* h1d_para_ccbar = new TH1D("h1d_para_ccbar","Parameters Distribution",100,0,1);
    for(int j = 0; j < parameters_ccbar.size(); j++) h1d_para_ccbar->Fill(parameters_ccbar[j]);
    ...
    c5->SetLogy();
    h1d_pair_pt_data_original->Draw();
    h1d_pair_pt_data_sys_original->Draw("same");
    ...
    TFile* fout = new TFile("template_fits_pair_pt_sys_upper.root","RECREATE");
    fout->cd();
