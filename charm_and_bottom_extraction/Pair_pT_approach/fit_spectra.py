...
x = np.loadtxt("pair_pt_data.txt", usecols=0, skiprows=0, dtype='float')
y = np.loadtxt("pair_pt_data.txt", usecols=1, skiprows=0, dtype='float')
err_y = np.loadtxt("pair_pt_data.txt", usecols=2, skiprows=0, dtype='float')
...
while count < 300:
    ...
    regr = MLPRegressor(max_iter=10000, solver='lbfgs', activation='relu').fit(X, Y) #'relu' logistic softmax tanh
    model_score = regr.score(np.atleast_2d(x).T, np.log10(y))

    if model_score < 0.90: #This matters a lot
        continue
...
np.savetxt("mlp_pair_pt_data.txt", myarray, fmt='%.5e', delimiter='\t', newline='\n', header='', footer='', comments='# ', encoding=None)
