...
void temp_script_prompt_ccbar_fixed(){

    fout = new TFile("template_fits_prompt_ccbar_fixed_upper.root","RECREATE");
    ...
    TFile* fccbar_stat = new TFile("normalizing_two_TGraphs_ccbar_stat.root","READ");
    TH1D* h1d_ccbar_central_stat = (TH1D*)fccbar_stat->Get("h1d_ccbar_central");
    h1d_ccbar_central_stat->SetName("h1d_ccbar_central_stat");
    TH1D* h1d_ccbar_lower_stat = (TH1D*)fccbar_stat->Get("h1d_ccbar_lower");
    h1d_ccbar_lower_stat->SetName("h1d_ccbar_lower_stat");
    TH1D* h1d_ccbar_upper_stat = (TH1D*)fccbar_stat->Get("h1d_ccbar_upper");
    h1d_ccbar_upper_stat->SetName("h1d_ccbar_upper_stat");

    TFile* fccbar_sys = new TFile("normalizing_two_TGraphs_ccbar_sys.root","READ");
    TH1D* h1d_ccbar_central_sys = (TH1D*)fccbar_sys->Get("h1d_ccbar_central");
    h1d_ccbar_central_sys->SetName("h1d_ccbar_central_sys");
    TH1D* h1d_ccbar_lower_sys = (TH1D*)fccbar_sys->Get("h1d_ccbar_lower");
    h1d_ccbar_lower_sys->SetName("h1d_ccbar_lower_sys");
    TH1D* h1d_ccbar_upper_sys = (TH1D*)fccbar_sys->Get("h1d_ccbar_upper");
    h1d_ccbar_upper_sys->SetName("h1d_ccbar_upper_sys");

    double scale_up = h1d_ccbar_upper_sys->Integral("width")/h1d_ccbar_central_stat->Integral("width");
    double scale_down = h1d_ccbar_lower_sys->Integral("width")/h1d_ccbar_central_stat->Integral("width");

    TH1D* h1d_ccbar_central = (TH1D*)h1d_ccbar_central_stat->Clone();
    h1d_ccbar_central->Scale(scale_up);
    h1d_ccbar_central->SetName("h1d_ccbar_central");

    TH1D* h1d_ccbar_lower = (TH1D*)h1d_ccbar_lower_stat->Clone();
    h1d_ccbar_lower->Scale(scale_up);
    h1d_ccbar_lower->SetName("h1d_ccbar_lower");

    TH1D* h1d_ccbar_upper = (TH1D*)h1d_ccbar_upper_stat->Clone();
    h1d_ccbar_upper->Scale(scale_up);
    h1d_ccbar_upper->SetName("h1d_ccbar_upper");
    ...
    for(int i = 0; i < mass_regions; i++){
        ...
        for(int ii = 0; ii < numTemplates; ii++){
            ...
            if(ii == 1){
                ...
                initial_fraction[ii] = h1d_ccbar_central->Integral(h1d_ccbar_central->FindBin(mass_lo[i]+0.01), h1d_ccbar_central->FindBin(mass_hi[i]-0.01),"width")/h1d_ee_FG_unlike->Integral(h1d_ee_FG_unlike->FindBin(mass_lo[i]+0.01), h1d_ee_FG_unlike->FindBin(mass_hi[i]-0.01),"width");
                min_fraction[ii] = h1d_ccbar_lower->Integral(h1d_ccbar_lower->FindBin(mass_lo[i]+0.01), h1d_ccbar_lower->FindBin(mass_hi[i]-0.01),"width")/h1d_ee_FG_unlike->Integral(h1d_ee_FG_unlike->FindBin(mass_lo[i]+0.01), h1d_ee_FG_unlike->FindBin(mass_hi[i]-0.01),"width");
                if(min_fraction[ii] < 0) min_fraction[ii] = 0.0;
                max_fraction[ii] = h1d_ccbar_upper->Integral(h1d_ccbar_upper->FindBin(mass_lo[i]+0.01), h1d_ccbar_upper->FindBin(mass_hi[i]-0.01),"width")/h1d_ee_FG_unlike->Integral(h1d_ee_FG_unlike->FindBin(mass_lo[i]+0.01), h1d_ee_FG_unlike->FindBin(mass_hi[i]-0.01),"width");
    ...
    h1d_ccbar_central->Write();
    h1d_ccbar_lower->Write();
    h1d_ccbar_upper->Write();
