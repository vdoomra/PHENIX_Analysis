...
x = np.loadtxt("plotting_full_ccbar.txt", usecols=0, skiprows=0, dtype='float')
y = np.loadtxt("plotting_full_ccbar.txt", usecols=1, skiprows=0, dtype='float')
err_y = np.loadtxt("plotting_full_ccbar.txt", usecols=2, skiprows=0, dtype='float')
...
while count < 200:
    ...
    if model_score < 0.50: #This matters a lot
        continue
    
    count = count + 1

    xpred = np.atleast_2d(np.linspace(1.50, 3.50, 200)).T
...
np.savetxt("mlp_full_ccbar.txt", myarray, fmt='%.5e', delimiter='\t', newline='\n', header='', footer='', comments='# ', encoding=None)
